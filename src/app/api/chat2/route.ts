...
const systemParts = {
    ...
    expectations: [
        `You will be asked questions about a family schedule and asked to provide information, as well as answer questions about the family.`,
...
<globalRules>
    ...
    <role>
        ${systemParts.role.join("\n")}
    </role>
    <personality>
        ${systemParts.personality.join("\n")}
    </personality>
    <expectations>
        ${systemParts.expectations.join("\n")}
    </expectations>
    <responses>
        ${systemParts.responses.join("\n")}
    </responses>
    <specialCommands>
        ${systemParts.special_commands.join("\n")}
        <exampleCommands>
            ${systemParts.example_commands.join("\n")}
        </exampleCommands>
    </specialCommands>    
</globalRules>
<knowledge>
    <family datatype="json">
        \`\`\`${JSON.stringify(family)}\`\`\`
    </family>
    <events datatype="json">
        \`\`\`${stringifiedEvents}\`\`\`
    </events>
    <packingLists datatype="json">
        \`\`\`${JSON.stringify(packingLists)}\`\`\`
    </packingLists>
    <now>
        <day>
