        ...
        <ul className={`ml-4 list-none`}>
          {events.map((event, index) => {
            ...
            <li key={index} className={`text-${size === 'lg' ? 'md' : 'sm'} text-raisin-black flex flex-row justify-between gap-2`}>
              ...
              <span className={`${event?.adjustments?.length ? 'font-semibold' : ''} ${event?.adjustments?.includes("CANCELLED") ? 'line-through' : ''} text-ellipsis overflow-hidden whitespace-nowrap`}> {event.title}
              </span>
              <span>
              {event.familyMembers.map(member => (
                <span key={`${member.name}-${event.title}-${event.start}`} className={`text-${size === 'lg' ? 'sm' : 'xs'} inline-flex items-center justify-center flex-row ${member.role === 'Parent' ? 'bg-mint' : 'bg-sea-buckthorn'} text-white rounded-full aspect-square px-0.5 py-0.5 mx-0 w-4 h-4`}>
